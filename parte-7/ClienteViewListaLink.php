...
foreach ($dados as $valor) {  
	...
	if (isset($valor['telefone'])) {
		$telefone=preg_replace("/\D/","",$valor['telefone']);
		$telefone=preg_replace("/(\d{2})(\d+)(\d{4})$/", "($1)$2-$3", $telefone); 
	} else $telefone=null;	
?>	
	<tr>
		<td><a href='<?php print "$url";?>'><?php print $valor['nome'];?></a></td>
		<td><a href='<?php print "$url";?>'><?php print $valor['telefone'];?></a></td>
		<td><a href='<?php print "$url";?>'><?php print $valor['email'];?></a></td>
	</tr>	
<?php	
